...
void BasicTaskSchedulerEpoll::moveSocketHandling(int oldSocketNum, int newSocketNum)
{
    ...
    if (oldSocketNum < 0 || newSocketNum < 0 || fEpollFd == -1)
        return; // sanity check
...
#if 1
void BasicTaskSchedulerEpoll ::setBackgroundHandling(int socketNum, int conditionSet, BackgroundHandlerProc *handlerProc, void *clientData)
{
    int ret = 0;
    struct epoll_event ev;
    memset(&ev, 0, sizeof(struct epoll_event));

    if (socketNum < 0 || fEpollFd == -1)
        return ;
